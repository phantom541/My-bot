...
const { getGroupSettings, updateGroupSettings, loadGroupSettings } = require('./groupSettings.js');
...
const server = http.createServer((req, res) => {
  ...
  res.end(JSON.stringify({
    status: 'WhatsApp Dragon Bot is running!',
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`HTTP server running on port ${PORT} for workflow compatibility`);
});
...
const CARD_PACK_SIZE = parseInt(process.env.CARD_PACK_SIZE) || 3;
const GUILD_CREATE_COST = parseInt(process.env.GUILD_CREATE_COST) || 10000;
...
async function main() {
  ...
  sock.ev.on('messages.delete', async ({ keys }) => {
    if (botSettings.antideleteEnabled) {
        console.log('Message deleted:', keys);
    }
  });
  ...
  sock.ev.on('messages.upsert', async (m) => {
    ...
    const context = {
        sock, msg, args, from, sender, player, savePlayer, reply, isGroupAdmin, hasRole, commandName,
        activeWildEncounters, activeBattles, activeTrades, quests, dungeons, market, achievements,
        botSettings, activeCardSpawns, activeCardPacks, activeTournaments, activeDungeons, activeBeast,
